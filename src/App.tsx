...
function NavBar({ active, setActive }: { active: string; setActive: (id: string) => void }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 py-4">
      <div className="max-w-5xl mx-auto rounded-2xl px-6 py-3 flex items-center justify-between"
        style={{ background: 'rgba(7,9,15,0.88)', backdropFilter: 'blur(20px)', border: '1px solid rgba(251,191,36,0.12)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="hidden md:flex items-center gap-3">
              ...
              <button onClick={logout} className="text-white/30 hover:text-white/60 transition-colors" title="Выйти">
                <Icon name="LogOut" size={16} />
              </button>
            </div>
          ) : (
            <button className="hidden md:block btn-gold px-5 py-2 text-xs rounded-xl" onClick={() => setActive('auth')}>
              Войти
            </button>
          )}
          <button className="md:hidden text-white/60 hover:text-gold-400 transition-colors" onClick={() => setMenuOpen(!menuOpen)}>
        ...
        <div className="md:hidden mt-2 max-w-5xl mx-auto rounded-2xl overflow-hidden"
          ...
          {user ? (
            <button onClick={() => { logout(); setMenuOpen(false); }}
              ...
              <span className="font-oswald tracking-wider uppercase text-sm">Выйти</span>
            </button>
          ) : (
            <button onClick={() => { setActive('auth'); setMenuOpen(false); }}
              className="w-full flex items-center gap-3 px-6 py-4 text-gold-400 border-t border-white/5">
              <Icon name="LogIn" size={18} />
              <span className="font-oswald tracking-wider uppercase text-sm">Войти</span>
            </button>
          )}
        </div>
      )}
...
function AppInner() {
  ...
  const renderPage = () => {
    switch (activePage) {
      case 'home': return <HomePage onGoToDoors={() => setActivePage('doors')} />;
      case 'doors': return <DoorsPage onNeedAuth={() => setActivePage('auth')} />;
      case 'cabinet': return <CabinetPage onGoAuth={() => setActivePage('auth')} />;
      case 'admin': return <AdminPage onGoAuth={() => setActivePage('auth')} />;
      case 'about': return <AboutPage />;
      case 'contacts': return <ContactsPage />;
      case 'auth': return <AuthPage onSuccess={() => setActivePage('cabinet')} initialRef={initialRef} />;
      default: return <HomePage onGoToDoors={() => setActivePage('doors')} />;
    }
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: '#07090f' }}>
      ...
      <footer className="border-t border-white/5 py-8 px-4 text-center">
        <div className="font-oswald text-xs text-white/20 tracking-widest uppercase">© 2026 Golden Door · Все права защищены</div>
        <div className="flex justify-center gap-6 mt-3">
          {['Условия', 'Политика', 'Правила'].map(link => (
            <button key={link} className="text-xs text-white/20 hover:text-white/40 font-rubik transition-colors">{link}</button>
          ))}
        </div>
      </footer>
    </div>
